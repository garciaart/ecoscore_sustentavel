...
from scoring import ScoringSystemEmpresas
from tips import TipsSystemEmpresas
from dashboard import DashboardEmpresas
        ...
        if not auth.get_empresa_logada():
            ...
            if opcao == "1":
                if habits.exibir_formulario(empresa.id):
                    ...
                    if resultado:
                        print(f"\n🎯 Score da Empresa: {resultado['total']}/100")
                ...
                if pontuacoes:
                    pontuacao_recente = max(pontuacoes, key=lambda p: p.data)
                    tips.gerar_relatorio_completo(empresa, pontuacao_recente.score_total, pontuacao_recente.nivel_sustentabilidade)
                else:
...
def exibir_ranking_empresas(db):
    ...
    for empresa in empresas:
        ...
        if pontuacoes:
            ...
            ranking.append((empresa, ultima_pontuacao.score_total, ultima_pontuacao.nivel_sustentabilidade))
    ...
    for i, (empresa, score, nivel) in enumerate(ranking[:10], 1):
        ...
        print(f"   {medal} {empresa.nome_fantasia} - {score}/100 - {nivel}")
